...
public interface PromotionRepository extends JpaRepository<Promotion, Integer> {
	...
	@Query("select m.promotion from Team t join t.customers c join t.localMatches vm join vm.markets m where c.id = ?1 and m.promotion.startMoment < NOW() and m.promotion.endMoment > NOW() and m.promotion.cancel = false")
